...
This document outlines the end-to-end data pipeline and architectural decisions made for the Eco-Monitor Pro system.
...
The trained model artifacts (`.pkl`) and extracted insights (`feature_importances.json`, `metrics.json`) are decoupled from the training scripts. The Streamlit application (`app.py`) loads these frozen assets into memory, ensuring highly responsive UI interactions. We utilize Plotly to generate interactive maps, radar charts, and correlation heatmaps to provide maximum transparency into both the data and the model mechanics.
