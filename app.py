...
def fetch_open_meteo_daily(lat: float, lon: float) -> dict:
    return get_open_meteo_daily(lat, lon)
...
def render_weather_forecast(city: str):
    ...
    with st.spinner(f"Pulling live weather data for {city}..."):
        daily_features = fetch_open_meteo_daily(coords["lat"], coords["lon"])
        forecast = fetch_weather_forecast(coords["lat"], coords["lon"])
        
        if "daily" in daily_features and str(forecast.get("cod")) == "200":
            ...
            try:
                ...
                with res_col2:
                    st.markdown("### ☁️ Standard API Forecast")
